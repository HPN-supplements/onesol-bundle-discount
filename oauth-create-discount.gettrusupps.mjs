/**
 * One-time OAuth helper to create the Bundle Builder Discount for gettrusupps.
 *
 * 1. Run:  node oauth-create-discount.gettrusupps.mjs
...
const CLIENT_ID     = '8d79d2769a9d4e38f1dac60f2717c57b';
const CLIENT_SECRET = process.env.SHOPIFY_CLIENT_SECRET;
const SHOP          = 'gettrusupps.myshopify.com';
const SCOPES        = 'write_discounts,read_discounts';
const REDIRECT_URI  = 'http://localhost:3456/callback';
const FUNCTION_GID  = 'gid://shopify/ShopifyFunction/019d4a4c-bb74-720f-a775-78e7bca420c4';
...
async function createDiscount(token) {
  ...
  if (!bundleFn) {
    console.log('\n❌ Could not find bundle function. Trying with hardcoded GID...');
    // Fall back to hardcoded
    return createDiscountWithGid(token, FUNCTION_GID);
  }

  console.log(`\n✅ Found function: "${bundleFn.title}" → ${bundleFn.id}`);
  return createDiscountWithGid(token, `gid://shopify/ShopifyFunction/${bundleFn.id}`);
}
